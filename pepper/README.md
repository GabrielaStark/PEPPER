...
| [rehydrate/](rehydrate/) | agente `rehidratador-legacy` | `pepper validate` para `environment.json` | runner de recetas pendiente |
...
Archivos transversales: `cli.py` (comandos), `session.py` (session.json), `profiles.py` (localizar y cargar perfiles), `detect.py`, `validate.py`.
