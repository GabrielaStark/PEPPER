...
- Si el recorrido se interrumpe (Ctrl+C, un error del navegador), la sesión queda escrita igual con lo reunido y `explore` sale con 1: repite con otro `--session`; no hace falta bajar el entorno. `--budget <segundos>` acota el recorrido y cierra limpio.
