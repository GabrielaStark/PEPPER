...
Cuando la ruta es la raíz de un repo con PEPPER instalado encima (`.`), la herramienta convive con el legacy: **ignora** `.claude/`, `pepper/`, `schemas/`, `profiles/`, `docs/documentacion/`, `docs/pepper/`, `pepper-out/`, `evidence/`, `CLAUDE.md` y `AGENTS.md` — no son artefactos del sistema. `pepper detect` ya los excluye por su cuenta.
