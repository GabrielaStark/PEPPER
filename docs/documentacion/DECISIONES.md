...
- **Decisión**: la red del compose es `internal: true`; el puerto del app se publica mediante un proxy de entrada (`ingress`, que es el proxy HTTP de PEPPER — D21). Todo host externo que el artefacto invoque se resuelve por alias DNS al stub. La receta de restauración re-apunta al stub cada servidor de `pg_foreign_server` (`dblink`, `postgres_fdw`).
...
- **Decisión**: tras la auditoría adversarial externa, tres garantías dejaron de ser convenciones y pasaron a ser mecánica del núcleo. (1) `pepper isolate` es **fail-closed**: el veredicto es VERIFICADO / NO AISLADO / **NO VERIFICADO**, y lo que no se pudo comprobar (compose sin resolver, sin contenedores vivos, red ilegible, hash del proxy sin verificar) bloquea igual que una fuga; el ingress ya no es un nombre sino una identidad verificada (imagen python + comando del proxy + un único montaje `:ro` con SHA-256 idéntico a `pepper/proxy.py`), y ningún servicio puede montar el socket de Docker, correr privileged, agregar capacidades ni compartir namespaces del host. (2) Correlate escribe `evidence-manifest.json` (SHA-256 de toda su salida), Package lo re-mapea verificando cada copia, y Export **exige y verifica** el manifest — evidencia alterada, fabricada o colada rechaza la publicación; `--manifest` acepta una copia conservada fuera del alcance del agente. (3) `jsonschema` es obligatorio para Export: D10 (nunca publicar inválidos) manda sobre D16 (sin dependencias).
...
- **Consecuencia aceptada**: más fricción legítima — sin Docker no hay verde de aislamiento (el fallback YAML da NO VERIFICADO), un paquete viejo sin manifest no exporta (se re-empaqueta), y Export sin jsonschema se niega. La frontera de datos hacia agentes externos (clasificación, modo local, sanitización) queda declarada en el README del paquete pero su mecánica es del siguiente bloque de endurecimiento — hasta entonces, ningún dato institucional real.
