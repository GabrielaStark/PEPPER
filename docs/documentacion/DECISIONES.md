...
- **Consecuencia aceptada**: el entorno reproduce la configuración de un perfil que puede no ser el de producción (p. ej. `alterno` en vez de `prod`); se declara cuál se usó y qué difiere. Y el stub convierte cada llamada externa en un error rápido: los flujos que dependen de esos servicios se observan solo hasta ese punto.
