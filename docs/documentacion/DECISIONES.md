...
- **Decisión**: la red del compose es `internal: true`; todos los servicios, incluido el ingress, se conectan únicamente a ella. El host entra por un puerto del proxy publicado en `127.0.0.1`; eso no requiere conectar el ingress a una red externa. Todo host externo que el artefacto invoque se resuelve por alias DNS al stub. La receta de restauración re-apunta al stub cada servidor de `pg_foreign_server` (`dblink`, `postgres_fdw`).
